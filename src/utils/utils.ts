...
export async function getLLMSSetting(knowledgeId: number | string) {
  ...
  const SettingRS: any[] = await new Promise((resolve, reject) => {
    db.all(
      "SELECT name,content from setting where type='openaisetting' and knowledgeId='" +
        knowledgeIdFilter +
        "' and userId='" +
        userIdFilter +
        "'",
...
export async function setOpenAISetting(Params: any) {
  ...
  try {
    const insertSetting = db.prepare(
      'INSERT OR REPLACE INTO setting (name, content, type, knowledgeId, userId) VALUES (?, ?, ?, ?, ?)'
    )
...
export async function getTemplate(knowledgeId: number | string) {
  ...
  const SettingRS: any[] = await new Promise((resolve, reject) => {
    ...
    db.all(
      "SELECT name,content from setting where type='" +
        Templatename +
        "' and knowledgeId='" +
        knowledgeIdFilter +
        "' and userId='" +
...
export async function addKnowledge(Params: any) {
  try {
    ...
    const RecordId = await new Promise((resolve, reject) => {
      db.get(
        "SELECT id from knowledge where name = '" + filterString(Params.name) + "' and userId = '" + userIdFilter + "'",
        (err: any, result: any) => {
          if (err) {
            reject(err)
          } else {
    ...
    if (RecordId) {
      ...
      const insertSetting = db.prepare(
        'INSERT OR REPLACE INTO knowledge (name, summary, timestamp, userId) VALUES (?, ?, ?, ?)'
      )
...
export async function uploadfilesInsertIntoDb(files: any[], knowledgeId: number | string) {
  ...
  const insertFiles = db.prepare(
    'INSERT OR IGNORE INTO files (knowledgeId: number | string, suffixName, newName, originalName, hash, timestamp, userId) VALUES (?,?,?,?,?,?,?)'
  )
  filesInfo.map((Item: any) => {
    ...
    fs.rename(
      DataDir + '/uploadfiles/' + Item.newName,
      DataDir + '/uploadfiles/' + String(userId) + '/' + String(knowledgeId) + '/' + Item.newName,
      (err: any) => {
        if (err) {
...
export async function getFilesPage(pageid: number, pagesize: number) {
  ...
  const RecordsAll: any[] = await new Promise((resolve, reject) => {
    db.all(
      'SELECT * from files where 1=1 order by status desc, timestamp desc limit ' +
        Number(pagesize) +
        ' offset ' +
        From +
        '',
...
export async function getFilesKnowledgeId(knowledgeId: number | string, pageid: number, pagesize: number) {
  ...
  const RecordsTotal: number = await new Promise((resolve, reject) => {
    db.get(
      "SELECT COUNT(*) AS NUM from files where knowledgeId = '" + KnowledgeIdFiler + "'",
      (err: any, result: any) => {
        if (err) {
          reject(err)
        } else {
  ...
  const RecordsAll: any[] = await new Promise((resolve, reject) => {
    db.all(
      "SELECT * from files where knowledgeId = '" +
        KnowledgeIdFiler +
        "' order by status desc, timestamp desc limit " +
        Number(pagesize) +
        ' offset ' +
        From +
        '',
...
export async function getChatLogByKnowledgeIdAndUserId(
  ...
  const RecordsTotal: number = await new Promise((resolve, reject) => {
    db.get(
      "SELECT COUNT(*) AS NUM from chatlog where knowledgeId = '" +
        KnowledgeIdFiler +
        "' and userId = '" +
        userIdFiler +
        "'",
  ...
  const RecordsAll: any[] = await new Promise((resolve, reject) => {
    db.all(
      "SELECT * from chatlog where knowledgeId = '" +
        KnowledgeIdFiler +
        "' and userId = '" +
        userIdFiler +
...
export async function getLogsPage(pageid: number, pagesize: number) {
  ...
  const RecordsAll: any[] = await new Promise((resolve, reject) => {
    db.all(
      'SELECT * from logs where 1=1 order by id desc limit ' + pagesizeFiler + ' offset ' + From + '',
      (err: any, result: any) => {
        if (err) {
          reject(err)
        } else {
...
export async function getKnowledgePage(pageid: number, pagesize: number) {
  ...
  const RecordsAll: any[] = await new Promise((resolve, reject) => {
    db.all(
      "SELECT * from knowledge where userId='" +
        userIdFiler +
...
export async function log(
  ...
  const content =
    ...
    ' ' +
    JSON.stringify(Action4) +
    ' ' +
    JSON.stringify(Action5) +
    ' ' +
    JSON.stringify(Action6) +
    ' ' +
    JSON.stringify(Action7) +
    ' ' +
    JSON.stringify(Action8) +
    ' ' +
    JSON.stringify(Action9) +
    ' ' +
    JSON.stringify(Action10)
  const insertStat = db.prepare(
    'INSERT OR REPLACE INTO logs (datetime, content, knowledgeId, userId) VALUES (? ,? ,? ,?)'
  )
...
export async function GetSetting(Name: string, knowledgeId: number | string, userId: number) {
  return await new Promise((resolve, reject) => {
    db.get(
      "SELECT content FROM setting where name='" +
        Name +
        "' and knowledgeId='" +
        knowledgeId +
        "' and userId='" +
